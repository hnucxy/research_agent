...
from utils.failure_experience import retrieve_failure_warnings
...
class PlannerNode:
    ...
    def _retrieve_success_experience(
        self, user_request: str, current_func: str, replan_count: int
    ):
    ...
    def _retrieve_failure_warning_context(
        self, user_request: str, current_func: str, search_source: str
    ) -> str:
        preferred_tools = []
        if current_func == "a":
            ...
            preferred_tools.append("literature_rag_search")

        try:
            return retrieve_failure_warnings(
                query=user_request,
                embeddings=self.embeddings,
                preferred_tools=preferred_tools,
            )
        except Exception:
            logger.warning("    [Planner] 失败经验库检索跳过或无预警。")
            return "无"
    ...
    def _build_strategy_hint(
        ...
        if failure_warning_context != "无":
            strategy_parts.append(
                "失败经验库已给出预警。规划时优先避开这些死胡同，不要重复使用已知会导致空结果或参数报错的模式。"
            )

        return "\n".join(strategy_parts) if strategy_parts else "无"
