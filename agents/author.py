...
AUTHOR_PROMPT = """你是一位严谨的学术论文作者。你的任务是结合用户的需求及审稿专家的反馈进行撰写或润色。
...
【要求】:
1. 如果有【审稿专家反馈】，请务必针对反馈中的意见进行逐条修改。
2. 如果上方提供了【当前已有草稿】，请在此原稿的基础上进行修改润色，切勿完全脱离原稿框架。
3. 如果【参考文档内容】不为“无”，必须严格参考其中的事实；如果为“无”，请仅基于常识和【当前已有草稿】进行学术化语言的重写和逻辑梳理，不要捏造外部实验数据。
4. 请直接输出学术文本，不要有任何寒暄、废话或解释性前缀。
"""
...
class AuthorNode:
    ...
    def __call__(self, state: AgentState) -> dict:
        ...
        current_draft = state.get("current_draft", "无")
